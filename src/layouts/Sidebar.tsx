...
import { ShieldCheck, LayoutDashboard, Calendar, Layers, BookOpen, UserCircle, Gamepad2, FileText } from 'lucide-react';
...
const Sidebar: React.FC = () => {
    ...
    return (
        <nav className="w-full md:w-72 bg-slate-900 text-white flex-shrink-0 flex flex-col shadow-2xl z-20 h-screen sticky top-0">
            ...
            <div className="flex-1 p-4 flex flex-col gap-2 overflow-y-auto">
                {navItems.map((item) => (
                    <NavLink
                        ...
                        to={item.to}
                        className={({ isActive }) =>
                            `flex items-center gap-3 p-3 rounded-xl transition-all ${isActive ? 'bg-blue-600 shadow-lg' : 'hover:bg-slate-800'}`
                        }
                    >
                        {item.icon}
