...
const Documents: React.FC = () => {
    const documents = [
        ...
        { title: "Manual de Funciones y Competencias (Decreto 815)", type: "PDF", size: "1.8 MB", locked: true },
        ...
        <div className="space-y-8 animate-in fade-in duration-500">
            <div className="flex justify-between items-center mb-6">
                <div>
                    ...
                    <p className="text-slate-500 mt-1">Recursos oficiales para el estudio autónomo.</p>
                </div>
            </div>
                ...
                {documents.map((doc, i) => (
                    <div key={i} className={`p-4 rounded-xl border flex items-center justify-between group transition-all duration-300 ${doc.locked ? 'bg-slate-50 border-slate-100 opacity-70' : 'bg-white border-slate-200 hover:border-blue-300 hover:shadow-md'}`}>
                        <div className="flex items-center gap-4">
                            <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${doc.locked ? 'bg-slate-200 text-slate-400' : 'bg-red-50 text-red-500'}`}>
